...
const TemplatePreview: React.FC<TemplatePreviewProps> = ({
  ...
  const renderClassicChronoTemplate = () => (
    <div className={`bg-white shadow-lg ${isPreview ? 'w-64 h-80' : 'max-w-[8.5in] mx-auto'} ${baseScale}`}>
      <div className={`${isPreview ? 'p-2' : 'p-8'}`}>
        {/* Header */}
        <div className={`text-center border-b ${isPreview ? 'pb-1 mb-2' : 'pb-6 mb-8'}`}>
          <h1 className={`font-bold text-gray-900 ${isPreview ? headerScale : 'text-3xl'} ${isPreview ? 'mb-0.5' : 'mb-2'}`}>
        ...
        {/* Summary */}
        <div className={isPreview ? 'mb-1' : 'mb-6'}>
        ...
        <div className={isPreview ? 'mb-1' : 'mb-6'}>
          ...
          {resumeData.experience.map((exp, index) => (
            <div key={exp.id} className={isPreview ? 'mb-1' : 'mb-4'}>
              <div className={`flex justify-between items-start ${isPreview ? 'mb-0.5' : 'mb-2'}`}>
                <div>
                  <h4 className={`font-semibold text-gray-900 ${titleScale}`}>{exp.title}</h4>
              ...
              <ul className={`space-y-1 ${isPreview ? 'ml-1' : 'ml-4'}`}>
                {exp.bullets.slice(0, isPreview ? 2 : exp.bullets.length).map((bullet, bulletIndex) => (
        ...
        <div className={isPreview ? 'mb-1' : 'mb-6'}>
          ...
          {resumeData.education.map((edu, index) => (
            <div key={edu.id}>
              <h4 className={`font-semibold text-gray-900 ${titleScale}`}>{edu.degree}</h4>
        ...
        <div>
          ...
          <div className={`flex flex-wrap ${isPreview ? 'gap-0.5' : 'gap-2'}`}>
            {resumeData.skills.slice(0, isPreview ? 4 : resumeData.skills.length).map((skill, index) => (
              <span key={index} className={`bg-gray-100 text-gray-700 rounded-full ${isPreview ? 'px-1 py-0.5' : 'px-3 py-1'}`}>
                {skill}
              </span>
            ))}
          </div>
  ...
  const renderModernMinimalistTemplate = () => (
    <div className={`bg-white shadow-lg border-l-4 border-blue-500 ${isPreview ? 'w-64 h-80' : 'max-w-[8.5in] mx-auto'} ${baseScale}`}>
      <div className={`${isPreview ? 'p-2' : 'p-8'}`}>
        {/* Header */}
        <div className={isPreview ? 'mb-2' : 'mb-8'}>
          <h1 className={`font-bold text-gray-900 ${isPreview ? headerScale : 'text-3xl'} ${isPreview ? 'mb-0.5' : 'mb-2'}`}>
            {resumeData.personalInfo.name}
          </h1>
  ...
  const renderPlainProTemplate = () => (
    <div className={`bg-white shadow-sm border ${isPreview ? 'w-64 h-80' : 'max-w-[8.5in] mx-auto'} ${baseScale}`}>
      <div className={`${isPreview ? 'p-2' : 'p-8'}`}>
        {/* Header */}
        <div className={`text-center ${isPreview ? 'mb-2' : 'mb-6'}`}>
          <h1 className={`font-bold text-gray-900 ${isPreview ? headerScale : 'text-2xl'} ${isPreview ? 'mb-0.5' : 'mb-1'}`}>
            {resumeData.personalInfo.name}
          </h1>
        ...
        {/* Summary */}
        <div className={isPreview ? 'mb-1' : 'mb-4'}>
        ...
        <div className={isPreview ? 'mb-1' : 'mb-4'}>
          ...
          <hr className={isPreview ? 'mb-0.5' : 'mb-2'} />
          {resumeData.experience.map((exp, index) => (
            <div key={exp.id} className={isPreview ? 'mb-1' : 'mb-3'}>
              <div className="flex justify-between">
                <h4 className={`font-bold text-gray-900 ${titleScale}`}>{exp.title}</h4>
        ...
        <div className={isPreview ? 'mb-1' : 'mb-4'}>
          ...
          {resumeData.education.map((edu, index) => (
            <div key={edu.id} className="flex justify-between">
              <div>
                <h4 className={`font-bold text-gray-900 ${titleScale}`}>{edu.degree}</h4>
        ...
        <div>
          ...
          <hr className={isPreview ? 'mb-0.5' : 'mb-2'} />
          <p className="text-gray-700">{resumeData.skills.slice(0, isPreview ? 4 : resumeData.skills.length).join(', ')}</p>
        </div>
      </div>
    </div>
  );
  ...
  const renderSmartAccentTemplate = () => (
    <div className={`bg-white shadow-lg ${isPreview ? 'w-64 h-80' : 'max-w-[8.5in] mx-auto'} ${baseScale}`}>
      <div className={`${isPreview ? 'p-2' : 'p-8'}`}>
        {/* Header with accent */}
        <div className={`bg-gradient-to-r from-purple-600 to-blue-600 text-white ${isPreview ? 'p-1 mb-2' : 'p-6 mb-6'} -mx-8 -mt-8`}>
          <h1 className={`font-bold ${isPreview ? headerScale : 'text-3xl'} ${isPreview ? 'mb-0.5' : 'mb-2'}`}>
            {resumeData.personalInfo.name}
          </h1>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <div className={`flex flex-wrap ${isPreview ? 'gap-0.5' : 'gap-1'}`}>
              {resumeData.skills.slice(0, isPreview ? 4 : resumeData.skills.length).map((skill, index) => (
                <span key={index} className={`bg-purple-100 text-purple-700 rounded ${isPreview ? 'px-1 py-0.5' : 'px-2 py-1'}`}>
                  {skill}
                </span>
              ))}
            </div>
  ...
  const renderSideStripeTemplate = () => (
    <div className={`bg-white shadow-lg grid grid-cols-3 gap-0 ${isPreview ? 'w-64 h-80' : 'max-w-[8.5in] mx-auto'} ${baseScale}`}>
      {/* Sidebar */}
      <div className={`bg-gray-800 text-white col-span-1 ${isPreview ? 'p-1' : 'p-6'}`}>
        <div className={isPreview ? 'mb-1' : 'mb-6'}>
          <h1 className={`font-bold ${isPreview ? headerScale : 'text-2xl'} ${isPreview ? 'mb-0.5' : 'mb-1'}`}>
            {resumeData.personalInfo.name}
          </h1>
          <div className={`space-y-1 ${isPreview ? 'text-[3px]' : 'text-sm'}`}>
        ...
        <div>
          ...
          {resumeData.education.map((edu, index) => (
            <div key={edu.id} className={`${isPreview ? 'text-[3px]' : 'text-sm'}`}>
      ...
      {/* Main Content */}
      <div className={`col-span-2 ${isPreview ? 'p-1' : 'p-6'}`}>
        <div className={isPreview ? 'mb-1' : 'mb-6'}>
  ...
  const renderElegantContrastTemplate = () => (
    <div className={`bg-white shadow-lg ${isPreview ? 'w-64 h-80' : 'max-w-[8.5in] mx-auto'} ${baseScale}`}>
      <div className={`${isPreview ? 'p-2' : 'p-8'}`}>
        {/* Header */}
        <div className={`bg-gray-900 text-white ${isPreview ? 'p-1 mb-2' : 'p-6 mb-6'} -mx-8 -mt-8`}>
          <h1 className={`font-bold ${isPreview ? headerScale : 'text-3xl'} ${isPreview ? 'mb-0.5' : 'mb-2'}`}>
            {resumeData.personalInfo.name}
          </h1>
        ...
        <div className={isPreview ? 'mb-1' : 'mb-6'}>
          ...
          {resumeData.experience.map((exp, index) => (
            <div key={exp.id} className={isPreview ? 'mb-1' : 'mb-4'}>
              <div className={`border-l-4 border-gray-900 ${isPreview ? 'pl-1' : 'pl-4'}`}>
                <div className={`flex justify-between items-start ${isPreview ? 'mb-0.5' : 'mb-2'}`}>
                  <div>
                    <h4 className={`font-semibold text-gray-900 ${titleScale}`}>{exp.title}</h4>
                    <div className="text-gray-600 font-medium">{exp.company}</div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            {resumeData.education.map((edu, index) => (
              <div key={edu.id} className={`border-l-4 border-gray-300 ${isPreview ? 'pl-1' : 'pl-4'}`}>
                <h4 className={`font-semibold text-gray-900 ${titleScale}`}>{edu.degree}</h4>
                <div className="text-gray-600">{edu.school}</div>
          ...
          <div>
            ...
            <div className={`border-l-4 border-gray-300 ${isPreview ? 'pl-1' : 'pl-4'} space-y-1`}>
              {resumeData.skills.slice(0, isPreview ? 4 : resumeData.skills.length).map((skill, index) => (
                <div key={index} className="text-gray-700">• {skill}</div>
              ))}
            </div>
